 ...
 *
 * This endpoint implements the standard MCP-over-HTTP protocol:
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    const request: JSONRPCRequest = await context.request.json()
    ...
    if (!request.method) {
      return jsonrpcError(request.id, -32600, "Invalid Request: method is required")
    }
    ...
    switch (request.method) {
      ...
      case "tools/call":
        ...
        result = await handleToolCall(db, name, args || {})
        break
    ...
    return jsonrpcError(null, -32603, `Internal error: ${error.message}`)
  }
}

/**
 * Handle tool call - maps to the existing implementation from call.ts
 */
async function handleToolCall(db: any, tool: string, args: any) {
  try {
    let result: any
...
/**
 * Helper functions - extracted from call.ts
 */
...
// GET endpoint for MCP server discovery
export const GET: APIRoute = async () => {
  return new Response(JSON.stringify({
    name: "xmail",
    version: "1.0.0",
    description: "XMail MCP Server - Email management and verification code automation",
    protocol: "mcp",
    transport: "http",
    capabilities: {
      tools: true
