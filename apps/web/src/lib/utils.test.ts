...
describe('extractVerificationCode', () => {
  describe('正确提取验证码', () => {
    ...
    it('应该提取法语验证码邮件', () => {
      const text = 'Votre code de vérification est 593214. Il expire dans 10 minutes.'
      expect(extractVerificationCode(text, null)).toBe('593214')
    })

    it('应该提取德语验证码邮件', () => {
      const text = 'Ihr Sicherheitscode lautet 741963. Bitte geben Sie ihn ein.'
      expect(extractVerificationCode(text, null)).toBe('741963')
    })

    it('应该提取弱关键词的西语动作句式验证码', () => {
      const text = 'Utilice 927351 para continuar con el inicio de sesión.'
      expect(extractVerificationCode(text, null)).toBe('927351')
    })

    it('应该提取包含样式和URL噪声的验证码', () => {
      const text = "Your OpenAI code is 603686 @font-face { font-family: 'Söhne'; src: url(https://cdn.openai.com/common/static/soehne-buch.woff2) format('woff2'); }"
...
`
  ...
  describe('避免误判其他数字', () => {
    ...
    it('不应该把优惠券码识别成验证码', () => {
      const text = 'Use coupon 123456 to get 10% off on your next order.'
      expect(extractVerificationCode(text, null)).toBeNull()
    })

    it('不应该把欢迎邮件里的URL短码识别成验证码', () => {
      const text = 'Welcome to ChatGPT Business. Invite your team now with URL3243.'
      expect(extractVerificationCode(text, null)).toBeNull()
    })
  })
