...
const twoFactorSchemaState = new WeakMap<object, Promise<void>>()
...
// 简单的文本加密/解密（使用 Base64 + XOR）
// 注意：这不是强加密，仅用于混淆存储
function encryptStoredText(value: string): string {
  // 使用 Base64 编码
...
function decryptStoredText(encrypted: string): string {
  try {
...
function encryptPassword(password: string): string {
  return encryptStoredText(password)
}

function decryptPassword(encrypted: string): string {
  return decryptStoredText(encrypted)
}

async function ensureTwoFactorSchema(db: DB): Promise<void> {
  const client = db.$client as object
  const existing = twoFactorSchemaState.get(client)
  if (existing) {
    await existing
    return
  }
  ...
  const pending = (async () => {
    await db.run(`CREATE TABLE IF NOT EXISTS two_factor_entries (
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS user_two_factor_entries (
      user_id TEXT NOT NULL,
      two_factor_entry_id TEXT NOT NULL,
      assigned_at INTEGER NOT NULL,
      PRIMARY KEY (user_id, two_factor_entry_id),
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
      FOREIGN KEY (two_factor_entry_id) REFERENCES two_factor_entries(id) ON DELETE CASCADE
    )`)
    await db.run(`CREATE INDEX IF NOT EXISTS idx_two_factor_entries_updated_at ON two_factor_entries(updated_at)`)
    await db.run(`CREATE INDEX IF NOT EXISTS idx_user_two_factor_entries_user_id ON user_two_factor_entries(user_id)`)
  })()

  twoFactorSchemaState.set(client, pending)
  try {
    await pending
  } catch (error) {
    twoFactorSchemaState.delete(client)
    throw error
  }
}
...
export async function deleteUser(db: DB, id: string): Promise<void> {
  ...
  await db.delete(schema.userTwoFactorEntries).where(eq(schema.userTwoFactorEntries.userId, id))
...
export async function createTwoFactorEntry(db: DB, data: {
  name: string
  secret: string
  issuer?: string | null
...
}): Promise<schema.TwoFactorEntry> {
  await ensureTwoFactorSchema(db)
  const timestamp = now()
  const entry: schema.InsertTwoFactorEntry = {
    id: nanoid(),
    name: data.name,
    issuer: data.issuer ?? null,
    accountName: data.accountName ?? null,
    encryptedSecret: encryptStoredText(data.secret),
  ...
  await db.insert(schema.twoFactorEntries).values(entry)
  return entry as schema.TwoFactorEntry
}

export async function updateTwoFactorEntry(db: DB, id: string, data: {
  name?: string
  secret?: string
  issuer?: string | null
  accountName?: string | null
  ...
  await ensureTwoFactorSchema(db)
  const updateData: Partial<schema.InsertTwoFactorEntry> = {
    updatedAt: now(),
  }

  if (data.name !== undefined) updateData.name = data.name
  if (data.secret !== undefined) updateData.encryptedSecret = encryptStoredText(data.secret)
  ...
  await db.update(schema.twoFactorEntries).set(updateData).where(eq(schema.twoFactorEntries.id, id))
}

export async function deleteTwoFactorEntry(db: DB, id: string): Promise<void> {
  await ensureTwoFactorSchema(db)
  await db.delete(schema.userTwoFactorEntries).where(eq(schema.userTwoFactorEntries.twoFactorEntryId, id))
  await db.delete(schema.twoFactorEntries).where(eq(schema.twoFactorEntries.id, id))
}

export async function getTwoFactorEntry(db: DB, id: string, options: { includeSecret?: boolean } = {}): Promise<(schema.TwoFactorEntry & { secret?: string }) | null> {
  await ensureTwoFactorSchema(db)
  const entry = await db.select().from(schema.twoFactorEntries).where(eq(schema.twoFactorEntries.id, id)).get()
  if (!entry) return null
  if (!options.includeSecret) return entry

  return {
    ...entry,
    secret: decryptStoredText(entry.encryptedSecret),
  }
}

export async function listTwoFactorEntries(db: DB, options: { includeSecret?: boolean } = {}): Promise<Array<schema.TwoFactorEntry & { secret?: string }>> {
  await ensureTwoFactorSchema(db)
  const entries = await db.select().from(schema.twoFactorEntries)
    .orderBy(desc(schema.twoFactorEntries.updatedAt), desc(schema.twoFactorEntries.createdAt))
    .all()

  if (!options.includeSecret) return entries
...
export async function assignTwoFactorEntryToUsers(db: DB, id: string, userIds: string[]): Promise<void> {
  await ensureTwoFactorSchema(db)
  await db.delete(schema.userTwoFactorEntries).where(eq(schema.userTwoFactorEntries.twoFactorEntryId, id))
  if (userIds.length === 0) return

  await db.insert(schema.userTwoFactorEntries).values(
    userIds.map((userId) => ({
      userId,
      twoFactorEntryId: id,
      assignedAt: now(),
...
export async function getTwoFactorEntryUsers(db: DB, id: string): Promise<schema.UserTwoFactorEntry[]> {
  await ensureTwoFactorSchema(db)
...
export async function listTwoFactorEntriesForUser(db: DB, userId: string): Promise<Array<schema.TwoFactorEntry & { secret: string }>> {
  ...
  const entryIds = assignments.map((assignment) => assignment.twoFactorEntryId)
  const entries = await db.select().from(schema.twoFactorEntries)
    .where(inArray(schema.twoFactorEntries.id, entryIds))
    .orderBy(desc(schema.twoFactorEntries.updatedAt), desc(schema.twoFactorEntries.createdAt))
    .all()
