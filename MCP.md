...
XMail 现在支持 Model Context Protocol (MCP)，让 AI 助手可以直接调用工具进行邮箱自动化操作。

## 什么是 MCP？

Model Context Protocol (MCP) 是一个开放标准，用于让 AI 应用（如 Claude Desktop）与外部工具和数据源集成。通过 MCP，AI 可以：
...
如果你使用 Claude Code CLI，可以使用命令直接添加 MCP 服务器：

```bash
# 添加 XMail MCP 服务器
claude mcp add xmail \
...
# 测试连接
claude mcp test xmail
```

#### 方式二：手动编辑配置文件（Claude Desktop）

如果你使用 Claude Desktop，需要手动编辑配置文件：
...
Claude 会调用 `get_verification_code` 工具，自动从该邮箱的最新邮件中提取验证码并返回。
...
```bash
# 添加 MCP 服务器
claude mcp add <name> --url <url> --header "Authorization: Bearer <key>"
...
3. **最小权限原则**：如果只需要查询功能，考虑在未来版本中使用只读 API Key（待实现）。
...
3. 考虑修改 `apps/web/src/lib/utils.ts:extractVerificationCode()` 算法
