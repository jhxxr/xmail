...
**如果你在使用 Claude Code，只需一句话即可完成部署：**

```
帮我完成 XMail 的完整部署：
1. 创建 D1 数据库并执行迁移
2. 生成 JWT_SECRET 和 ADMIN_PASSWORD
3. 配置 wrangler.toml 中的 database_id 和 MAIL_DOMAIN
4. 部署 Email Worker 和 Web 应用
5. 提醒我配置 Email Routing
```

Claude Code 会自动执行所有命令、生成密钥、修改配置、完成部署。

**其他常用自动化场景：**

```bash
# 批量创建邮箱
"创建 20 个邮箱 test01-test20@example.com，设置随机密码并导出 CSV"

# 批量注册助手
"创建 10 个邮箱，我提交注册后你每 10 秒检查验证码并生成报告"

# 运维操作
"删除所有 30 天前的邮件，统计清理数量"
```
